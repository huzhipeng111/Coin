...
//    }
    ...
    fun i(vararg objects: Any) {
        if (LogLevel.INFO.value <= logLevel.value) {
               val s = getMethodNames()
               Log.i(TAG, String.format(s, getObjectsString(objects)))
        }
    }
...
//                .append(LoggerPrinter.BOTTOM_BORDER).append(LoggerPrinter.BR)
    ...
    fun getClassName(string: String): String {
    ...
    private fun getObjectsString(vararg objects: Any): String {
        ...
        if (objects.size > 1) {
            ...
            for (i in objects.indices) {
                val objectin = objects[i]
                if (objectin == null) {
                    stringBuilder.append(PARAM).append("[").append(i).append("]").append(" = ").append(NULL).append("\n")
                } else {
                    stringBuilder.append(PARAM).append("[").append(i).append("]").append(" = ").append(objectin.toString()).append("\n")
                }
            }
